...
/// reach the Java side. Nothing below knows anything about WPF beyond calling <see cref="WpfSpike"/>.
/// </summary>
[Activity(
    Label = "WPF Android Spike",
    MainLauncher = true,
...
public class MainActivity : Activity
{
    protected override void OnCreate(Bundle? savedInstanceState)
    {
        ...
        EnableDiagnostics(FilesDir!.AbsolutePath);
        ...
        // real content size rather than a zero one. (Post runs on the next Looper turn, by which
        // point the content view has been measured; a late Surface would fix the size up anyway.)
        root.Post(WpfSpike.Run);
    }

    /// <summary>
    /// Turn on the engine's diagnostics, OFF by default because they are not free: the sink log is
    /// an AppendAllText (open/write/close) per line with several lines per frame, and switching it on
    /// also enables the renderer's per-frame PERF accounting. Enable per run with
    ///     adb shell setprop debug.wpf.sinklog 1        (engine log -> files/wpf-sink.log)
    ///     adb shell setprop debug.wpf.wgpulog debug    (wgpu's own backend trace, very verbose)
    /// </summary>
    internal static void EnableDiagnostics(string filesDir)
    {
        if (AndroidSystemProperties.Get("debug.wpf.sinklog") is "1" or "true")
            System.Environment.SetEnvironmentVariable("WPF_WEBGPU_SINK_LOG", System.IO.Path.Combine(filesDir, "wpf-sink.log"));
