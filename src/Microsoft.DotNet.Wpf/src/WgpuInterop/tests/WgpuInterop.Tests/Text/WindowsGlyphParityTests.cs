    ...
    public sealed class WindowsGlyphParityTests : RendererTestBase
    {
       ...
       [Fact]
        ...
        public void HowOurWeightTracksGdis()
        {
            ...
            // WPF_WEIGHT_FAMILIES=A|B|C replaces the six faces, and WPF_WEIGHT_BI=1 adds bold
            // italic as a fourth style -- the same comparison asked of faces and styles the
            // specimen never draws.
            string[] families = Environment.GetEnvironmentVariable("WPF_WEIGHT_FAMILIES") is { Length: > 0 } wf
                ? wf.Split('|')
                : new[] { "Segoe UI", "Arial", "Times New Roman", "Verdana", "Tahoma", "Consolas" };
            var styles = new List<(bool, bool)> { (false, false), (true, false), (false, true) };
            if (Environment.GetEnvironmentVariable("WPF_WEIGHT_BI") == "1") styles.Add((true, true));
            foreach (string family in families)
                foreach ((bool bold, bool italic) in styles)
                    ...
                    foreach (int ppem in WeightSizes)
                    {
                        ...
                        // WPF_WEIGHT_PIXDUMP=1: every differing pixel, GDI's RGB against ours.
                        if (Environment.GetEnvironmentVariable("WPF_WEIGHT_PIXDUMP") == "1")
                            for (int i = 0; i < SpecimenWidth * Height; i++)
                                if (raw[i * 4 + 2] != ours[i * 4] || raw[i * 4 + 1] != ours[i * 4 + 1]
                                    || raw[i * 4] != ours[i * 4 + 2])
                                    report.AppendLine($"      pix x={i % SpecimenWidth - PenX} y={i / SpecimenWidth - 28}"
                                        + $" gdi {raw[i * 4 + 2]},{raw[i * 4 + 1]},{raw[i * 4]}"
                                        + $" ours {ours[i * 4]},{ours[i * 4 + 1]},{ours[i * 4 + 2]}");
                        ...
                        bool columns = Environment.GetEnvironmentVariable("WPF_WEIGHT_COLUMNS")
                                       == $"{family}|{(bold && italic ? "BI" : bold ? "B" : italic ? "I" : "R")}|{ppem}";
                        ...
                        grandTotal += sumd;
                        rowCount++;
                        report.AppendLine($"   {family,-16} {(bold && italic ? "BI" : bold ? "B" : italic ? "I" : "R")}   {ppem,4}"
                                          + $" {mine,9} {theirs,9}"
                        ...
                        if (Environment.GetEnvironmentVariable("WPF_WEIGHT_PERGLYPH") == "1")
                            foreach (char c in System.Linq.Enumerable.Distinct(Sample))
                            {
                                ...
                                report.AppendLine($"      glyph {family}|{(bold && italic ? "BI" : bold ? "B" : italic ? "I" : "R")}|{ppem}|{c} {d1}");
                            }
                    }
