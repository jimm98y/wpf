    ...
    internal sealed partial class TrueTypeInterpreter
    {
        ...
        public bool Hint(GlyphProgram glyph, float pixelsPerEm)
        {
            ...
            try
            {
                ...
                // 0x8b IS NOT THE VALUE OF THIS WORD -- a retraction of the previous commit.
                // fs__NewTransformation at 1400261e0 does `w10 = 1 << (clientRec[0x488] - 1);
                // tst w10, #0x8b; ccmp w8, #0x1f` -- a VALIDITY CHECK that clientRec[0x488] is one
                // of {1, 2, 4, 8}, branching to an error otherwise. The word itself is stored at
                // 140026354 from a register set elsewhere in the flow. What does stand: every
                // tbz/tbnz on globals[0x1c0] in fsg_SimpleInnerGridFit, itrp_CALL,
                // itrp_DeltaEngine, itrp_ALIGNRP and itrp_GETINFO tests bit 0, 1 or 2, and all
