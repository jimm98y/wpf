...
// Browser async readback lives in WgpuSceneRenderer.Browser.cs (a non-unsafe
        ...
        private void EmitText(GlyphRunDraw run, Matrix3x2 world, double opacity, Scissor clip, int width, int height, WGPUTextureFormat format, DrawData data)
        {
            ...
            if (outline != null)
            {
                ...
                Dictionary<int, int>? batchDropout = null;
                int colL = 0, colR = 0;
                ...
                void FlushBatch()
                {
                    ...
                    PathRasterizer.GlyphDropoutForRun = batchDropout;
                    try
                    {
                    ...
                    finally
                    {
                        ...
                        PathRasterizer.GlyphDropoutForRun = null;
                    }
                ...
                foreach (Text.ShapedGlyph g in _shapeScratch)
                {
                    ...
                    if (hintPpem > 0f && font is Text.TrueTypeFont gdf2 && gdf2.GlyphDropout(g.GlyphId, hintPpem) is int gdv && gdv >= 0)
                        (batchDropout ??= new Dictionary<int, int>())[glyphOrdinal] = gdv;
                    ...
                    {
                        step = MathF.Round((g.Advance + g.Kern) * scale, MidpointRounding.AwayFromZero);
                    }
                    pen += step;
                }
            ...
            foreach (Text.ShapedGlyph sg in _shapeScratch)
            {
                ...
                penX += MathF.Round((sg.Advance + sg.Kern) * scale, MidpointRounding.AwayFromZero);
            }
        }
