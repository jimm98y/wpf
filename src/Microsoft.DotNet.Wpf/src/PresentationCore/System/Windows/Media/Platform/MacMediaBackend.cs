...
//
// MacMediaBackend -- the macOS IMediaBackend, backed by AVFoundation.
//
// AVPlayer decodes audio+video and drives the master A/V clock (HW decode via VideoToolbox). An
// AVPlayerItemVideoOutput (requesting 32BGRA) is the frame tap. A DispatcherTimer on the media dispatcher
// polls the item status (-> Opened/Failed) and, once ready, pulls the current CVPixelBuffer each tick and
// raises FrameAvailable; MediaPlayer.UpdateResource then locks that frame (TryLockFrame) and ships its BGRA
// pixels to the WebGPU compositor via the SendVideoFrame seam. Everything runs on the UI/dispatcher thread,
// so no cross-thread marshaling of frames is needed.
//
// Interop follows the fork's convention (see WgpuInterop MacInterop.cs): dlopen full framework paths, one
// typed objc_msgSend alias per call shape. CMTime is a 24-byte by-value struct -- on arm64 the plain
// objc_msgSend entry handles large struct returns/args, so the aliases below are correct for Apple Silicon.
//
...
namespace System.Windows.Media
{
    ...
    internal sealed class MacMediaBackend : IMediaBackend
    {
        ...
        internal MacMediaBackend(MediaPlayer player)
        {
        ...
        public void SetBalance(double balance) { /* AVPlayer has no balance knob; no-op (M5: AVAudioMix). */ }
        ...
        private void ReleaseFrame() => UnlockFrame();
        ...
        [DllImport(ObjC, EntryPoint = "objc_msgSend")] private static extern float SendFloat(IntPtr r, IntPtr s);
